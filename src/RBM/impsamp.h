...
public:
    Impsamp(double s_omega, double s_rho, int s_mc, int s_N, int s_dim, double s_dt, double sig, int s_H, bool s_interact, double s_spread);
    double energy_impsamp(const mat &R, double alpha);
    double best_alpha();
    double langevin(const vec &a, const vec &b, const mat &w, vec X, std::ofstream &myfile, ofstream &myfile2);
