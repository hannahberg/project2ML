...
//    double dt = 0.01;
    ...
    vec gammavec = {0.01, 0.05, 0.1, 0.5, 1.0, 2.0};
    //vec rhovec = {0.1,0.2,0.4,0.7,0.8,1.0};
    //vec dtvec = logspace<vec>(-4,0,51);
    double dt2 = 0.8;
    Solver S2(omega, rho, mc, numpart, howmanyDs, dt2, sig, hidden, interactionswitch, spread); // initialize Solver class
    vec b = S2.init_b(spread);
    mat w = S2.init_w(spread);
